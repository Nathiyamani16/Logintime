...
function AdminDashboard() {
  ...
  const handleEmployeeSubmit = async (e, departmentId) => {
    e.preventDefault();
    if (!employeeName || !employeeEmail || !employeeRole) return;

    const temporaryPassword = generateTemporaryPassword(); // Generate temporary password

    try {
      const response = await axios.post(`http://localhost:5000/departments/${departmentId}/employees`, {
        name: employeeName,
        email: employeeEmail,
        role: employeeRole,
        password: temporaryPassword, // Include the temporary password in the request
      });

      const { data } = response;
      setMessage(`Employee ${data.name} added successfully with role ${data.role}`);
      setEmployeeName('');
      setEmployeeEmail('');
      setEmployeeRole('');
    } catch (error) {
    ...
    <div>
      ...
      {departments.map((department, index) => (
        <div key={index}>
          ...
          <ul>
            {department.employees.map((employee, index) => (
              <li key={index}>{`${employee.name} - ${employee.email} - ${employee.role}`}</li>
            ))}
          </ul>
        </div>
      ))}
